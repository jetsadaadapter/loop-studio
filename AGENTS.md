...
**Auto-fulfill mode (opt-in).** A pending request can be fulfilled by a local agent without a human. `loop-bridge-worker.service.ts` holds an **adapter registry** (`ADAPTERS` — currently `claude` and `gemini`); the agent is chosen per-project via `LoopProject.autoAgent` (Edit Project modal), else by the `LOOP_BRIDGE_AUTO` server env default; unset = wait for a human. The worker spawns the selected CLI **read-only** — `claude`: `--permission-mode dontAsk --allowedTools Read,Grep,Glob` (no `--bare`, so it uses the machine's Claude login, keyless); `gemini`: `--approval-mode plan --skip-trust` (uses `GEMINI_API_KEY`). Read-only means the agent never writes project files itself — it returns `<file_edit>` blocks that the existing bridge POST route applies through the guarded `applyFileEdits`, so all edit guards (config lock, test-file policy, the task-scope guard — edits are confined to the task's `targetFiles` when set, with in-scope test siblings allowed for QA — the standard-path guard that refuses to auto-create a deep new directory tree from an agent-supplied path, `LOOP_MAX_NEW_DIR_LEVELS` default 1; and the per-project docs standard that confines a NEW markdown doc to the project's `docs/` dir, `LOOP_DOCS_DIR` default `docs` — root-level convention files, existing docs, `.github/`, and task-scoped paths exempt) still hold. Only registered adapters are ever spawned; the prompt is passed as a discrete argv element (no shell). Add a spawn agent by adding one `kind:"spawn"` entry to `ADAPTERS` (bin, argv builder, output parser).

**Agentic SDK adapter (`claude-sdk`).** A third adapter of `kind:"sdk"` runs an **in-process Agent SDK agentic loop** (`loop-sdk-runner.ts` → `runAgentSdk`, lazy-imported so the SDK's native binary loads only when used) instead of a read-only CLI. It **forces a task git worktree** (autonomous edits must be isolated — the SDK has no filesystem sandbox), then runs `query()` with the worktree as `cwd`, the guarded tools (`edit_file`/`run_verification` via `createSdkMcpServer`), and a **`PreToolUse` hook** as the choke point (the SDK's real gate — `canUseTool` is shadowed). `disallowedTools` removes the built-in `Write`/`Bash`/`Edit`; the hook reuses `evaluateEdit`/`evaluateToolCall` so config-lock / test-file / task-scope guards hold and RED/ORANGE risk tiers route to human review (the new-directory-depth cap is enforced at write time in `writeGuardedFile`, since it needs fs to know what already exists) (`ask`, which fails **closed** = deny when headless). Runs are capped by `LOOP_SDK_MAX_TURNS` (default 30) and `LOOP_SDK_MAX_BUDGET_USD` (default 5). Edits are applied + checkpointed in the worktree during the loop, so `finalizeBridgeReply` records the summary + `preAppliedFiles` (a `git diff --name-only` of the branch) rather than re-applying `<file_edit>` blocks. Auth is keyless (machine Claude login, verified) with `ANTHROPIC_API_KEY` as a fallback. See `docs/sdk-adapter-wiring.md` (+ `branch-per-task-checkpoint.md`, `guarded-tools-pretooluse.md`).
