                            ...
                            {drafts.map((d, i) => (
                                <li key={`${d.name}-${i}`} className="flex items-start justify-between gap-3 rounded-lg border border-slate-200/70 bg-slate-50/50 px-3 py-2">
                                    <div className="min-w-0">
                                        ...
                                        <p className="truncate text-[11px] text-slate-500" title={d.targetFiles.join(", ")}>{d.targetFiles.join(", ")}</p>
                                        <div className="mt-1 flex flex-wrap items-center gap-1">
                                            <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold font-sans ${riskColor(d.riskTier)}`}>{d.riskTier}</span>
                                            <span className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-sans text-slate-600">group {d.groupNumber}</span>
                                            {d.tags.map((t) => (
                                                <span key={t} className="rounded-full border border-indigo-200/60 bg-indigo-50 px-2 py-0.5 text-[10px] font-sans text-indigo-700">{t}</span>
                                            ))}
                                        </div>
                                    </div>
                                    <button
