...
import { Workflow, AlertCircle, Plus, SlidersHorizontal, List, LayoutGrid } from "lucide-react";
  ...
  const loadPrompts = () => {
    ...
    getManagePrompts({
      ...
      .then((res) => {
        ...
        setLastUpdated(new Date().toLocaleString("en-US", { month: "numeric", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit", second: "2-digit" }));
      })
      .catch((err) => {
    ...
    >
      ...
      <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-4 mb-6 select-none">
        ...
        <div className="flex flex-col xl:flex-row items-stretch xl:items-center gap-3 flex-1">
          <ManageSearchInput value={searchQuery} onChange={setSearchQuery} placeholder="Search prompts by name, description, content" />
          <div className="flex items-center gap-3 w-full xl:w-auto">
            <ManageFilterSelect label="Type" value={filterType} options={[{ value: "all", label: "All Types" }, { value: "system", label: "System Personas" }, { value: "user", label: "User Prompts" }]} onChange={setFilterType} width="xl:w-32" />
            <ManageFilterSelect label="Visibility" value={filterVisibility} options={[{ value: "all", label: "All Visibility" }, { value: "private", label: "Private" }, { value: "public", label: "Public" }]} onChange={setFilterVisibility} width="xl:w-28" />
            {(searchQuery || filterType !== "all" || filterVisibility !== "all") && (
              <Button type="button" variant="ghost" size="icon" onClick={() => { setSearchQuery(""); setFilterType("all"); setFilterVisibility("all"); }} className="size-8 rounded-sm border border-slate-200 hover:bg-slate-50 cursor-pointer text-slate-500 shadow-3xs flex items-center justify-center shrink-0" title="Reset Filters">
        ...
        <div className="flex items-center gap-3 justify-between xl:justify-end shrink-0 select-none">
          {lastUpdated && <span className="text-[10px] font-medium text-slate-400">Updated {lastUpdated}</span>}
          <ManageRefreshButton isLoading={isLoading} isRefreshing={isLoading} onRefresh={loadPrompts} title="Refresh Prompts" />
          <Button type="button" disabled={isLoading || isSaving} onClick={handleCreateClick} className="h-8 bg-brand hover:bg-brand/90 text-white text-xs font-semibold px-4.5 rounded-sm flex items-center gap-1.5 cursor-pointer shadow-sm shadow-brand/10 transition-all select-none flex-1 xl:flex-none justify-center">
            <Plus className="size-4 shrink-0" />Add Prompt
          </Button>
