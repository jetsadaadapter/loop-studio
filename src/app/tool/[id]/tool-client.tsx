...
import type { Tool, ToolJob, GetToolJobsResponse } from "@/core/interfaces/tools.interface";
...
const buildInitialForm = (params: Tool['params']): Record<string, unknown> => {
    ...
    params.forEach(param => {
        if (param.defaultValue !== null) {
            data[param.key] = param.type === 'boolean' ? param.defaultValue === 'true' : param.defaultValue;
        }
    });
    return data;
};
...
export function ToolClient({ tool, initialJobs }: ToolClientProps) {
    ...
    const [formData, setFormData] = useState<Record<string, unknown>>(() => buildInitialForm(tool.params));
                        ...
                        <div className="relative z-10 flex min-h-48 flex-col justify-end py-5 pt-10 text-white sm:min-h-64 sm:py-8 sm:pt-16 lg:min-h-80 lg:py-10 lg:pt-20">
                            <div className="max-w-3xl">
                                <div className="flex items-center gap-3 mb-2">
                                    <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-xs font-medium text-slate-300 ring-1 ring-white/10">AI Tool</span>
                                </div>
