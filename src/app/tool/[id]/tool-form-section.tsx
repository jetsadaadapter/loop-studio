    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" className="h-7.5 px-2.5 rounded-lg text-[10.5px] font-bold text-slate-600 hover:text-brand hover:bg-brand/5 border border-slate-200/50 shadow-xs bg-white transition-all duration-300 cursor-pointer hover:-translate-y-0.5 active:scale-[0.98] active:translate-y-0"
                    onClick={() => isBulk ? handleBulkCancel() : (setBulkValue(value.join('\n')), setIsBulk(true))}>
                    {isBulk ? <><List className="mr-1.5 size-3.5 text-slate-500" /> List View</> : <><FileText className="mr-1.5 size-3.5 text-slate-500" /> Bulk Edit</>}
                </Button>
            </div>
            {isBulk ? (
                <div className="space-y-3.5 p-4 rounded-2xl bg-slate-50/50 border border-slate-200/60 shadow-xs">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-2 bg-brand/5 border border-brand/10 rounded-xl text-[10px] text-slate-600 font-bold select-none">
                        ...
                        <span>Format: Paste one Facebook URL link per line, or separate links with commas.</span>
                    </div>

                    <div className="flex gap-2">
                        <Button size="sm" className="h-9 rounded-lg bg-gradient-to-r from-brand to-brand-strong hover:from-brand-strong hover:to-brand text-white text-xs px-5 shadow-md shadow-brand/25 font-bold transition-all duration-300 hover:shadow-lg hover:shadow-brand/40 hover:-translate-y-0.5 active:scale-[0.98] active:translate-y-0 cursor-pointer" onClick={handleBulkSave}>Apply Changes</Button>
                        <Button size="sm" variant="outline" className="h-9 rounded-lg text-xs px-5 border border-slate-200/80 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 font-bold shadow-xs transition-all duration-300 hover:-translate-y-0.5 active:scale-[0.98] active:translate-y-0 cursor-pointer" onClick={handleBulkCancel}>Cancel</Button>
                    </div>
                </div>
            ) : value.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-6 text-center bg-slate-50/30 rounded-2xl border border-dashed border-slate-200 shadow-xs relative overflow-hidden select-none group min-h-[140px] transition-all duration-300 hover:border-brand/40">
                    ...
                    <div className="mt-4 flex gap-2 w-full max-w-[280px] relative z-10 justify-center">
                        ...
                        <Button variant="outline" size="sm" className="h-8.5 px-3 rounded-lg border-brand/20 bg-brand/5 text-[10.5px] font-bold text-brand hover:bg-brand/10 hover:border-brand/30 transition-colors shadow-none cursor-pointer"
                            onClick={() => { setBulkValue(value.join('\n')); setIsBulk(true); }}>
                ...
                <div className="space-y-3">
                    {value.map((item, index) => (
                        <div key={`url-input-${id}-${index}`} className="flex gap-2 items-center group/row">
...
const PARAM_HELPERS: Record<string, string> = {
    starturls: "Enter a valid Facebook page URL, e.g. https://www.facebook.com/senatorWong/. Note that you can only scrape public pages with this Actor, not personal profiles.",
    ...
    onlynewerthan: "Scrape posts from the analyzed date to the present day (or dates in 'older than'). Supports YYYY-MM-DD absolute format or relative format (e.g. 1 day, 5 months) or UTC ISO timestamp.",
    onlyolderthan: "Scrape posts from the analyzed date to the past (or dates in 'newer than'). Supports YYYY-MM-DD absolute format or relative format (e.g. 1 day, 3 months) or UTC ISO timestamp."
};
...
export function ToolFormSection({ params, formData, errors, isRunning, onChange, onRun }: ToolFormSectionProps) {
    ...
    return (
        <Card className="shadow-[0_4px_20px_rgba(0,0,0,0.03)] border-slate-200/60 rounded-2xl overflow-hidden bg-white">
            <div className="bg-gradient-to-r from-slate-50/50 via-white to-slate-50/30 border-b border-slate-100/60 px-6 py-5 flex items-center gap-3">
                ...
                <div>
                    ...
                    <CardDescription className="mt-1 text-[11px] text-slate-400 font-medium">Configure the required parameters to start processing.</CardDescription>
                </div>
            </div>
                        ...
                        <Field key={param.id} className="space-y-2.5">
                            ...
                            {param.type === 'boolean' ? (
                                ...
                                <Select value={String(formData[param.key] || "")} onValueChange={(val) => clearError(param.key, val)}>
                                    <SelectTrigger id={param.key} className={cn("h-11 bg-slate-50 border-slate-200 focus:bg-white focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all text-sm rounded-xl", errors[param.key] && "border-red-500 focus:ring-red-500/20 bg-red-50/30")}>
                                ...
                                <Textarea id={param.key} placeholder={param.placeholder || `Enter ${param.label.toLowerCase()}...`}
                                    ...
                                    className={cn("min-h-[120px] py-3 bg-slate-50 border-slate-200/60 focus:bg-white focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all text-sm rounded-xl px-4 resize-none", errors[param.key] && "border-red-500 focus:ring-red-500/20 bg-red-50/30")} />
                            ) : (
                                <Input id={param.key} placeholder={param.placeholder || `Enter ${param.label.toLowerCase()}...`}
                                    value={String(formData[param.key] || "")} onChange={(e) => clearError(param.key, e.target.value)}
                                    className={cn("h-11 bg-slate-50 border-slate-200/60 focus:bg-white focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all text-sm rounded-xl px-4", errors[param.key] && "border-red-500 focus:ring-red-500/20 bg-red-50/30")} />
                            )}
